...
* `polygon3d(vertices)` draw a polygon with the given vertices, the modifier `triangulationMode` can be used to specify how the vertices are seperated into triangles, possible values are:
  ...
  - `TriangulateCenter` connect all vertices to additional vertex in center of polygon (mean of vertices)
...
* `triangles3d(triangles)` draw multiple (independent) triangles
  triangles can be given either as a list of vertices or as a list of lists containing three vertices each, the modifiers `normals`, `colors` and `uv` should contain one element (potentially a list) for each triangle

* `mesh3d(grid)` draws the rectangular mesh with the given sample points in grid, given as a rectangular matrix with in the form `grid_y_x`. It is expected that vertex data is given as rectangular grids of the same shape. The modifier `topology` can be used to specify how the gird should be closed, possible values are:
...
If the topology in a direction is closed it is possible to give one additional row/column of vertex modifiers in that direction to specify the vertex data of the end-points when approaching from the other side of the boundary.
...
Points lines and circles additionally have the modifier:
* `size` to specify the size (measured as a radius) of the drawn object
...
For cylinders and lines there are additional the following modifiers:
...
* `caps`, `cap1`, `cap2`: the style(s) use for the end-caps of the cylinder possible values are:
    - `CylinderCapOpen` cut orthogonal  to cylinder axis, draw back face (if visible)
    - `CylinderCapFlat` cut orthogonal to cylinder axis, draw a flat surface
    - `CylinderCapRound` cut orthogonal to cylinder axis, draw a round cap
    additionally the following values are used internally:
    - `CglCylinderCapVoid` cut orthogonal to cylinder axis, draw nothing
    - `CglCylinderCapCutOpen(normal)` cut orthogonal to given normal vector, draw back face (if visible)
    - `CglCylinderCapCutFlat(normal)` cut orthogonal to given normal vector, draw a flat surface
    - `CglCylinderCapCutVoid(normal)` cut orthogonal to given normal vector, draw nothing (used internally for "flat" joints)
    - `CglCylinderCapCutVoidRound(normal)` add round end-cap then cut orthogonal to given normal vector (used internally for round joints)

similarly connect and curve have the same cap modifiers as well as the modifiers:
...
* `vertexModifiers` a dictionary mapping variable names to list of values (one for each vertex). The variables behave similarly to plotModifiers with the difference that the value of the variable will be linearly interpolated between the values at the vertices of the current triangle.

Surfaces and plots allow specifying:
...
* `cutoffRegion` region in 3D-space where the surface/plot should be rendered possible values are:
  ...
  - `CutoffCube(center,sideLength,up,front)` a cube with the given center and side length with axes pointing in directions `up` and `front`
  - `CutoffCuboid(center,v1,v2,v3)` a cuboid with the given center and axes
  The function `cutoffAddPlane(cutoff,normal,depth)` can be used to intersect a cutoff region with the half-space `{P ; P*normal <= depth}`, the result will be another cutoff-region.
  The expressions `normal` and `depth` are reevaluated every frame, the modifier plot-modifiers `plotModifiers` can be used to define plot-modifiers used in the normal and depth expressions.
